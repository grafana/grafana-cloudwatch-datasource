...
function makeDatasource(): CloudWatchDatasource {
  return {
    resources: {
      getLogGroupFields(): Promise<ResourceResponse<LogGroupField[]>> {
...
class ValueMock {
  ...
  constructor(tokens: string | Token[], cursorOffset: number) {
    this.selection = {
      start: {
        offset: cursorOffset,
      },
 ...
 * @param tokens
 */
function addTokenMetadata(tokens: string | Token[]): Token[] {
  /* eslint-disable @typescript-eslint/no-explicit-any */
